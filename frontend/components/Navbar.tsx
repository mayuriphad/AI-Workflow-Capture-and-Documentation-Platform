...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="fixed inset-x-0 top-0 z-40 flex h-10 items-center justify-between bg-[#26343B] px-3 text-xs font-medium text-gray-300 shadow-md">
        <ToolTabsNav />

        {/* Right: Utility Icons -- same minimal icon-button language as the
            session top bar, kept functionally distinct since Home actually
            has real notifications where the session bar doesn't. */}
        <div className="flex h-full shrink-0 items-center gap-3.5 pr-1">
          <div className="relative" ref={notifRef}>
            ...
            <DropdownPanel
              open={notifOpen}
              className="absolute right-0 top-7 w-64 rounded bg-[#2D2D2D] border border-gray-700 shadow-xl overflow-hidden z-50"
            >
          ...
          <button className="text-gray-300 hover:text-white transition-colors" title="Help">
            <IconHelpCircle />
          </button>
          ...
          <button className="text-gray-300 hover:text-red-400 transition-colors" title="Close">
            <IconX />
          </button>
        </div>
      </nav>
